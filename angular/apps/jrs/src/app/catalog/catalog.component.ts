...
export class CatalogComponent implements OnInit {
  ...
  constructor(cartService: CartService) {
    ...
    this.filters = ['Heads', 'Arms', 'Torsos', 'Bases', 'All'];
  }
  ...
  public getFilteredProducts(): IProduct[] {
    // May be needed if getting products form a backend service and is not immediately available
    // if (!this.products || this.products.length === 0) {
    //   return [];
    // }

    return !this.currentFilter || this.currentFilter === '' || this.currentFilter === 'All'
      ? this.products
      : this.products.filter((p) => p.category.toLowerCase() === this.currentFilter.toLowerCase());
  }
